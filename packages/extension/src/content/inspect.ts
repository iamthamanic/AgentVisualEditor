 ...
 * agent-preview.ts (INV-1). Idempotent boot so scripting.executeScript is safe.
 */
...
const BOOT_FLAG = "__aveInspectBooted";

if (Reflect.get(globalThis, BOOT_FLAG) !== true) {
  Reflect.set(globalThis, BOOT_FLAG, true);
  bootInspectContentScript();
}

function bootInspectContentScript(): void {
  ...
  function onClick(event: MouseEvent): void {
    ...
    // Hide blue box before SW captures the viewport preview.
    hideOverlay();
  ...
  // Sync inspect state if the side panel enabled it before this script loaded.
